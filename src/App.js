...
function App() {
  const [liste,setListe]=useState([{nom:"1er element",poids:10,gain:20},{nom:"2eme element",poids:20,gain:20}]);
  ...
  let g =  document.querySelector("#f_gain").value;
    ...
    {
      setListe(prevState => ([...prevState, {nom:eleNum()+" élément",poids:parseInt(pds),gain:parseInt(g)}]));  
...
function def ()
{
  ...
  return (
    ...
    <main>
      <button id="myBtn" onClick = {function() {document.getElementById("myModal").style.display = "block"}}>Ajouter un élément</button>
        ...
        <div className="modal-content">
          <span className="close" onClick = {function() {document.getElementById("myModal").style.display = "none"}}>&times;</span>
            <form method="POST" action="/" onSubmit={handleSubmit} noValidate>
              ...
              <span>
                <label htmlFor="gain">Le gain du {eleNum()} élément </label>
                <input type="number" name="gain" placeholder="Le gain" id="f_gain"/>
              </span>
      ...
      <button id="myBtn2" onClick = {function() {document.getElementById("myModal2").style.display = "block"}}>Redéfinir le poids maximal</button>
      <div id="myModal2" className="modal">
        ...
        <section>
          <div>
            <h5>Le probléme est le suivant : nous avons un cartable pouvant contenir un poids maximal de {poidsMaxDef}kg et un nombre {liste.length} éléments comme suit : </h5>
    ...
    <footer>
      La compléxité de cet algoritme est : O(n)
    </footer>
    </div>
  );
}
